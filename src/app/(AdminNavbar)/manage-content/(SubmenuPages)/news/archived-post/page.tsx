...
interface ArchivedNewsPost {
    name: string;
}

const ARCHIVED_NEWS_POSTS: ArchivedNewsPost[] = [
    { name: "Low Sugar Froyo Combos to Try at Yogurtland" },
]
            ...
            <Paper elevation={0} sx={{ border: "1px solid #e5e7eb", borderRadius: 2, overflow: "hidden" }}>
                <TableContainer>
                    <Table>
                        ...
                        <TableBody>
                            {ARCHIVED_NEWS_POSTS.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={2} align="center" sx={{ py: 8, color: "#9ca3af" }}>
                                        <Typography variant="body1">No archived news posts found.</Typography>
                                    </TableCell>
                                </TableRow>
                            ) : (
                                ARCHIVED_NEWS_POSTS.map((post, index) => (
                                    <TableRow
                                        key={`${post.name}-${index}`}
                                        sx={{
